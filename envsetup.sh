...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^rockos_") ; then
        ROCKOS_BUILD=$(echo -n $1 | sed -e 's/^rockos_//g')
        export BUILD_NUMBER=$( (date +%s%N ; echo $ROCKOS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10 )
    else
        ROCKOS_BUILD=
    fi
    export ROCKOS_BUILD
...
function print_lunch_menu()
{
    ...
    if [ "z${ROCKOS_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    ROCKOS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/rockos/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the ROCKOS model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch rockos_$target-$variant
        fi
    fi
    return $?
}
...
function repopick() {
    T=$(gettop)
    $T/vendor/rockos/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $ROCKOS_FIXUP_COMMON_OUT ]; then
...
function installboot()
{
    ...
    if (adb shell getprop ro.rockos.device | grep -q "$ROCKOS_BUILD");
    then
        ...
        echo "The connected device does not appear to be $ROCKOS_BUILD, run away!"
    fi
}
...
function dopush()
{
    ...
    if (adb shell getprop ro.rockos.device | grep -q "$ROCKOS_BUILD") || [ "$FORCE_PUSH" == "true" ];
    then
...
. $ANDROID_BUILD_TOP/vendor/rockos/build/envsetup.sh
